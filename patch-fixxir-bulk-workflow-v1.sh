...
if grep -q 'FIXXIR_BULK_WORKFLOW_V1' "$CODE_FILE" && \
   grep -q 'FIXXIR_BULK_WORKFLOW_UI_V1' "$INDEX_FILE"; then
  echo "Bulk workflow update is already installed."
  exit 0
fi

cp "$CODE_FILE" "${CODE_FILE}.before-bulk-workflow-v1.bak"
cp "$INDEX_FILE" "${INDEX_FILE}.before-bulk-workflow-v1.bak"
...
if "function ensureSaleProcurementForSale_(salesId)" not in code:
    ...
    extra_backend = r'''/* FIXXIR_BULK_WORKFLOW_V1 */
...
# --- Repair edit modal ---
html = re.sub(
    r'''(?is)<div>\s*<label class=["']required["']>Repair date</label>\s*
<input[^>]*id=["']editRepairDate["'][^>]*>\s*</div>''',
    '''<div>
...
html = re.sub(
    r'''(?is)\s*<div class=["']full["']>\s*
<label>Notes</label>\s*
<textarea[^>]*id=["']editRepairNotes["'][^>]*></textarea>\s*
</div>''',
    "",
    html,
    count=1,
...
# Marker.
if "FIXXIR_BULK_WORKFLOW_UI_V1" not in html:
    html = html.replace(
        "</style>",
        "  /* FIXXIR_BULK_WORKFLOW_UI_V1 */\n  </style>",
        1,
    )

# Validation.
required_code = [
    "FIXXIR_BULK_WORKFLOW_V1",
    "function updateSale(payload)",
    "function postSaleExpense(payload)",
    "function ensureSaleProcurementForSale_(salesId)",
    "function updatePurchaseHeader(payload)",
...
repair_edit_end = html.find('id="saleModal"', repair_edit_start)
repair_edit_region = html[repair_edit_start:repair_edit_end]
